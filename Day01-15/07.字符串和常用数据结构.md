...
### 生成式和生成器 Compehresion[] VS Generator()
...
set1.add(4)
set1.add(5)
set2.update([11, 12]) #add to the end of tuple
set2.discard(5)
if 4 in set2:
    set2.remove(4)
print(set1, set2)
print(set3.pop()) #pop(): remove random element in the set
print(set3)
```
...
print(set1 - set2) #elements in set1 but no in set2
...
print(scores)
# 创建字典的构造器语法
items1 = dict(one=1, two=2, three=3, four=4) #constructor method, have to use '=" ? can I use ':' ?
# 通过zip函数将两个序列压成字典
...
# 删除字典中的元素
print(scores.popitem()) #pop last insert item after version 3.7, pop random item before version 3.7
